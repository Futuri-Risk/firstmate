...
# bin/fm-pr-merge.sh addresses GitHub by owner/repository. Gitea and GitLab leave
# them empty: Gitea operations use the validated canonical URL, while GitLab
# addresses the project by FM_PR_HOST and FM_PR_PATH.
fm_pr_url_parse() {
  ...
  case "$raw" in
    */pulls/*)
      ...
      gitea_identity=$(printf '%s' "$gitea_identity" | jq -er '[.host,.path,(.number|tostring)] | @tsv') || return 1
      IFS=
...
fm_pr_poll_merge_notified_remove() {  # <state> <id>
  ...
  rm -f -- "$marker"
	...
	' read -r FM_PR_HOST FM_PR_PATH FM_PR_NUMBER <<< "$gitea_identity"
      FM_PR_PROVIDER=gitea
      FM_PR_URL=$raw
      return 0
      ;;
