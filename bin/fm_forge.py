...
def run(*args, cwd=None, env=None, input=None, timeout=120):
    ...
    if result.returncode:
        # A runtime warning on stderr must not hide a structured stdout error.
        # Redact before bounding the message, otherwise truncation can expose
        # the unmatched tail of a credential crossing the size boundary.
        ...
        raise ForgeError(f"{Path(str(args[0])).name} exited {result.returncode}: {error[-2000:].strip()}")
